...
export const AddResults = ({
  ...
  const [costPerResults, setCostPerResults] = useState<{ id: number, title: string, value: number }[]>([])
  const [results, setResults] = useState<{ id: number, title: string, value: number }[]>([])
    ...
    <div>
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} fullWidth>
        <DialogTitle fontSize={20}>
          Relatório do
          <Typography color="primary">{networkName}</Typography>
        </DialogTitle>
        <DialogContent>
